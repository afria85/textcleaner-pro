...
class StateManager {
    ...
    async initialize(options = {}) {
        ...
        try {
            console.info('Initializing StateManager...');
     ...
     */
    saveStateDebounced = this.debounce(() => {
        ...
        try {
            ...
            console.debug('State auto-saved');
        } catch (error) {
    ...
    setState(path, value, options = {}) {
        ...
        console.debug(`State updated: ${path}`, {
            oldValue,
            newValue,
            options,
        });
    ...
    subscribe(path, callback, options = {}) {
        ...
        console.debug(`Subscribed to state: ${path}`, { subscriptionId });
    ...
    unsubscribe(path, subscriptionId) {
        ...
        if (success) {
            console.debug(`Unsubscribed from state: ${path}`, { subscriptionId });
        }
                ...
                for (const [, { callback, options }] of parentSubscribers) {
                    if (options.deep) {
                        try {
                            ...
                            console.error(`Error in parent subscription callback for ${parentPath}:`, error);
                        }
                    }
                }
            }
    ...
    clearHistory() {
        this.history = [];
        this.historyIndex = -1;
        console.debug('Cleared state history');
    }
    ...
    use(middleware) {
        ...
        console.debug('Added state middleware');
    ...
    runMiddleware(type, data) {
        ...
        for (const middleware of this.middleware) {
            try {
                ...
                console.error(`Error in state middleware for ${type}:`, error);
            }
        }
    ...
    removeMiddleware(middleware) {
        ...
        if (index !== -1) {
            ...
            console.debug('Removed state middleware');
            return true;
        }
        return false;
    }
    ...
    reset(path = '') {
        if (path === '') {
            this.state.clear();
            this.clearHistory();
            console.debug('Reset entire state');
        } else {
            this.setState(path, undefined);
            console.debug(`Reset state: ${path}`);
        }
    }
                ...
                if (!notified.has(p) && this.subscribers.has(p)) {
                    ...
                    for (const [, { callback }] of subscribers) {
                        try {
                            ...
                            console.error(`Error in batch subscription callback for ${p}:`, error);
                        }
                    }
                    notified.add(p);
                }
